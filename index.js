...
function calculateCircleArea(radius) {
  if(isNaN(radius) || typeof radius !== `number`){
    console.log(error `: радіус має бути числом`);
    return null;
  }
...
function findMinMax(numbers) {
  if (!Array.isArray(numbers)){
    console.log(error `: переданий параметр не є масивом`);
    return null;
  }
...
return hypotenuse;
    ...
    console.log(`Помилка: довжина катетів має бути числами`);
    return null;
  }
...
value = Math.round(value);
   ...
   const roundObjectValues = Object.fromEntries(roundedEntries);
...
function calculateVolumeCylinder(radius, height) {
  ...
  const volumeCylinder =Math.ceil(Math.PI * Math.pow(radius ,2) * height );
...
function getFractionalPart(num) {
  ...
  const roundNum = Math.trunc(num);
  const diff = num - roundNum ;
  const roundDiff = Math.fround(diff);
  return roundDiff;
...
function compareAndRound(num1, num2) {
  ...
  const maxNum =Math.round( Math.max(num1, num2));
  return maxNum;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  && !isNaN(principal)  && !isNaN(years)  && !isNaN(interestRate)) {
    console.log( "Помилка: аргументи мають бути числами.");
    return null;
  }
  const sumPrincipal = Math.round( principal * Math.pow(1 + interestRate , years));
  return sumPrincipal;
